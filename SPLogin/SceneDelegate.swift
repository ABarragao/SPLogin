...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func initApp(window:UIWindow){
        ReachabilityManager.shared.reachability.whenUnreachable = nil
        ReachabilityManager.shared.reachability.whenReachable = nil
        ...
        if(UserDefaults.standard.getToken() == nil){
            let loginVc = LoginViewController(delegate: profileVC)
            loginVc.modalPresentationStyle = .fullScreen
            profileVC.present(loginVc, animated: false, completion: nil)
        }
    }
